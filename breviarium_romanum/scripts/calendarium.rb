...
date_enumerator.each do |date|
  if date.respond_to?(:month) && date.month != month
    ...
    puts '\end{parcolumns} }' if month != nil

    # month heading
    month = date.month
    puts "\\calMonth{#{MONTH_NAMES[month]}}\n\n"

    puts '{ \setlength{\parindent}{0pt}' # suppress indentation in group
    puts '\begin{parcolumns}[rulebetween=true,colwidths={1=5mm}]{2}'
  end
  ...
  unless celebrations.empty?
    ...
    celebrations.each_with_index do |celebration, ci|
      ...
      if celebration.local
        puts "\\emph{#{LOCAL_NAMES[celebration.local]}:}"
      end

      print "\\textsc{" if celebration.rank_code == :d2
      print "\\uppercase{" if celebration.rank_code == :d1
      print "#{celebration.title}."
      print "}" if [:d1, :d2].include? celebration.rank_code

      unless celebration.rank_code.nil? && celebration.title =~ /vigilia/i
        print " \\emph{#{rank_names[celebration.rank_code]}.}"
      end

      celebration.commemorations.each do |com|
        puts " \\emph{#{LOCAL_NAMES[com.local]}}" if com.local
        puts " \\emph{Commemoratio} #{com.title}."
      end
    end
    puts '}' # close colchunk
  end
...
puts '\end{parcolumns} }' # close last parcolumns environment
